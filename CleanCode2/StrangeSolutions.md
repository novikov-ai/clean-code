...
- Метод `GetVehicleStartEndTimeUnix` использовался для получения начала и конца временной отметки, но он также по сути являлся частным методом получения начала и конца. 
- Отрефакторил данные методы, сделав их более универсальными и проще поддерживаемыми (вынес в отдельный пакет)
